...
import type { Program } from '../features/programs/models/program.model';

/** Load user-created programs from localStorage (same key as ProgramStoreService). */
function getUserPrograms(): Program[] {
  try {
    const raw = localStorage.getItem('mereka_programs');
...
/** Merge fixture + user-created programs. User programs appear first. */
function allPrograms(): Program[] {
  return [...getUserPrograms(), ...PROGRAMS];
}
...
export function mockHandler(req: HttpRequest<unknown>): unknown | null {
  ...
  if (m) return all.find((p) => p.slug === m![1]) ?? null;

  // GET /api/v1/programs/:slug/curriculum
  m = path.match(/\/programs\/([^/]+)\/curriculum$/);
  if (m) {
    const p = all.find((p) => p.slug === m![1]);
